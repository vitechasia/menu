...
use Vdes\Menu\Models\Menus;
use Vdes\Menu\Models\MenuItems;
...
class WMenu
{
    ...
    public static function buildSideMenu($array)
    {
        foreach ($array as $item)
        {
            echo '<li '.(!empty($item['child']) ? "class=\"submenu\"" : "").' id="'.$item['id'].'"><a '.(!empty($item['child']) ? "" : "class=\"subdrop\"").' href="'.(!empty($item['child']) ? "javascript:void(0);" : $item['link']).'">';
            echo (!empty($item['child']) ? "<img src=\"".(($item['depth']>0) ? '' : (($item['class']==null) ? '' : url('/assets/img/icons').'/'.$item['class']))."\" alt=\"img\"><span> ".$item['label']."</span></span><span class=\"menu-arrow\"></span>" : "<span> ".$item['label']."</span>").'</a>';
            
            if (!empty($item['child']))
            {
                echo "<ul style=\"display: none;\">";
                    (new WMenu)->buildSideMenu($item['child']);
                echo '</ul>';
            }
            echo '</li>';
        }
